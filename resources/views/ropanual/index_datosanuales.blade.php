    ...
    @foreach($obj_tabla as $obj_item)
        ...
        @if(count($obj_param->def_columns['name_col']) > 0)
            @foreach($obj_param->def_columns['name_col'] as $col)
                @if ($col === 'id_laboratorios')
                    "{{ $obj_item->get_laboratorios['nombre_laboratorio'] }}",
                @elseif ($col === 'id_periodo_anual')
                    "{{ $obj_item->get_periodo_anual['desc_periodo_anual'] }}",
                @elseif ($col === 'id_meses')
                    "{{ $obj_item->get_meses['nombre_mes'] }}",
                @elseif ($col === 'id_tipo_prueba')
                    "{{ $obj_item->get_tipo_prueba['desc_tipo_prueba'] }}",
                @elseif ($col === 'id_cant_servicios')
                    "{{ $obj_item->get_cant_servicios['desc_cant_servicios'] }}",
                @elseif ($col === 'id_tipo_rrhh')
                    "{{ $obj_item->get_tipo_rrhh['desc_tipo_rrhh'] }}",
                @elseif ($col === 'id_tipo_beneficio')
                    "{{ $obj_item->get_tipo_beneficio['desc_tipo_beneficio'] }}",
    ...
    "select":   "single"
		...
		$("#ButtonEdit").click( function () {
			try {
	  			...
	  			var l_RecursoURL = "{{route('ropanual/edit_datosanuales')}}/"+l_Item;
			...
			try {
				var l_RecursoURL = "{{route('ropanual/delete_datosanuales','')}}";  
				var l_Item = table.row(".selected").data()[0]; 
			}
			catch(err) {
				swal({title: "Click en una fila para seleccionar", text: "Operacion Cancelada", icon:"error", buttons: false, timer:3000});

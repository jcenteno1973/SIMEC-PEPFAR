  ...
  <div class="modal-dialog modal-dialog-centered" role="document">      
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Municipio con el que se Trabajara</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
        
      {!! Form::open(['route' => 'administracion/setmunicipio', 'class' => 'form']) !!}
      <div class="modal-body">
          Seleccion de Municipio
          <input type="hidden" id="id_usuario_app" name="id_usuario_app" value="{{ Auth::user()->id_usuario_app }}">
            <select name="id_municipio" placeholder="id_municipio" class="form-control" >
                    @foreach(\App\Models\municipio::where('kpif','=',1)->get() as $obj_item)

...
<div class="panel panel-default">
    {!! Form::open(['route' => 'roptrimestre/index_telefono','autocomplete'=>'off']) !!}
    ...
    <div class="panel-body">
      ...
      <table class="table table-condensed">    
                <tr>
                    ...
                    <td>
                      {!!Form::text('numero_telefono',null,['class'=>'form-control', 'placeholder'=>'numero_telefono'])!!}   
                    </td>
                </tr>
     </table> 
    </div>
    ...
    <div>
        ...
        <a href="{{route('roptrimestre/create_telefono')}}/{{ $obj_filtro }}" class="btn btn-primary"><span class="fa fa-plus-circle" /> Adicionar</a>
...
<script>
    ...
    @foreach($obj_tabla as $obj_item)
        ["{{ $obj_item->id_telefono }}", "{{ $obj_item->id_index_testing }}", "{{ $obj_item->get_tipo->descripcion_tipo }}", "{{ $obj_item -> numero_telefono }}", "{{ $obj_item->ifenviarsms }}", "{{ $obj_item->ifprincipal }}"],
    @endforeach
];
    ...
    "select":   "single"
		...
		$("#ButtonEdit").click( function () {
			try {
	  			...
	  			var l_RecursoURL = "{{route('roptrimestre/edit_telefono')}}/"+l_Item;
			...
			try {
				var l_RecursoURL = "{{route('roptrimestre/delete_telefono','')}}";  
				var l_Item = table.row(".selected").data()[0]; 
			}
			catch(err) {
				swal({title: "Click en una fila para seleccionar", text: "Operacion Cancelada", icon:"error", buttons: false, timer:3000});

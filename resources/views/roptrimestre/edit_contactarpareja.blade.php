...
<div class="panel panel-default">
    <div class="panel-body">         
        <!--Crear nuevo rol-->
        <div class="col-lg-8">
             {!! Form::open(['route' => 'roptrimestre/edit_contactarpareja', 'class' => 'form','autocomplete'=>'off']) !!}
                <input type="hidden" id="id_contactar_pareja" name="id_contactar_pareja" value="{{ $obj_tabla->id_contactar_pareja }}">
                ...
                <table class="table table-condensed">
                      ...
                      <td>
                        <select onchange="ifSelected(event);" id="id_tipo_referencia" name="id_tipo_referencia" placeholder="id_tipo_referencia" class="form-control" required>
                            ...
                            @foreach($obj_select_tipo_referencia as $obj_item)
                                @if($obj_tabla->id_tipo_referencia == $obj_item->id_tipo_referencia)
                                    <option value="{{ $obj_item->id_tipo_referencia }}" selected>{{ $obj_item->tipo_referencia }} </option>
                                @else
                    ...
                    <tr>
                      <td>Fecha Limite</td>
                      <td><input type="text" maxlength="10" class="form-control datepicker" name="fecha_limite" id="fecha_limite" placeholder="dd/mm/aaaa" ></td>
                    </tr>

                    <tr>
                      <td>¿Contacto con Exito?</td>
                      <td><input type="checkbox" onclick="ifChecked(event);" class="custom-control-input" name="contacto_exito" id="contacto_exito" value="1" @if ($obj_tabla -> contacto_exito != 0) checked @endif></td>
                    </tr>
                    <tr>
                      <td>Fecha Contacto</td>
                      <td><input type="text" maxlength="10" class="form-control datepicker" name="fecha_contacto" id="fecha_contacto" placeholder="dd/mm/aaaa" value="{{ $obj_tabla->fecha_contacto->format('d/m/Y') }}" ></td>
                    </tr>

                    <tr>
                      <td>Pareja Acepto Realizar Prueba de VIH</td>
                      <td> 
                        <input type="radio" id="male" name="acepta" value="1" onchange="ifOption('male')" >
                        <label for="male" >Si</label><br>
                        <input type="radio" id="female" name="acepta" value="0" onchange="ifOption('female')" >
                    ...
                    <tr>
                      ...
                      <td>
                          <select name="id_motivo_rechazo_index" id="id_motivo_rechazo_index" class="form-control" >
                            <option value="" >-- Seleccionar -- </option>
                    ...
                    <tr>
                      <td>Observacion</td>
                      <td><input type="text" maxlength="250" class="form-control" name="observacion" id="observacion" placeholder="observacion" value="{{ $obj_tabla->observacion }}" ></td>
                    </tr>

...
<script>
    ...
    @foreach($obj_tabla as $obj_item)
        ["{{ $obj_item->id_contactar_pareja }}", "{{ $obj_item->id_index_testing }}", "{{ $obj_item->get_tipo_referencia -> tipo_referencia }}", "{{ $obj_item -> ifcontactoexito }}", "{{ $obj_item->feccontacto }}", "{{ $obj_item->observacion }}"],
    @endforeach
];
    ...
    "select":   "single"
		...
		$("#ButtonEdit").click( function () {
			try {
	  			...
	  			var l_RecursoURL = "{{route('roptrimestre/edit_contactarpareja')}}/"+l_Item;
			...
			try {
				var l_RecursoURL = "{{route('roptrimestre/delete_contactarpareja','')}}";  
				var l_Item = table.row(".selected").data()[0]; 
			}
			catch(err) {
				swal({title: "Click en una fila para seleccionar", text: "Operacion Cancelada", icon:"error", buttons: false, timer:3000});
